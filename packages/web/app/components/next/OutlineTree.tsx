...
import { loadDomainPageAction, loadGroupPageAction, searchAction } from "@/next-actions";
...
export function OutlineTree({ orgId, projectId, outline, selected, mapHref, view }: Props) {
  const hrefFor = (q: Record<string, string>) =>
    // Selecting a concept or group opens its ledger, whichever view you came from (review D1).
    `${mapHref}?${new URLSearchParams({ ...q, ...(view || Object.keys(q).length ? { view: "outline" } : {}) })}`;
  ...
  // Filtering searches the whole project on the server — not just the pages loaded here (review D4).
  const [hits, setHits] = useState<Array<{ id: string; label: string; key: string }> | null>(null);
  useEffect(() => {
    if (!f) return setHits(null);
    let live = true;
    const t = setTimeout(async () => {
      const r = await searchAction(orgId, projectId, f);
      if (live) setHits(r?.concepts ?? []);
    }, 200);
    return () => {
      live = false;
      clearTimeout(t);
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {f ? (
        <div className="min-h-0 flex-1 overflow-y-auto pb-6" aria-label="Matching concepts">
          {hits === null ? (
            <p className="px-4 py-2 text-[12px] text-faint">Searching…</p>
          ) : hits.length === 0 ? (
            <p className="px-4 py-2 text-[12px] text-faint">No concept matches “{filter}”.</p>
          ) : (
            <>
              {hits.map((c) => (
                <Link
                  ...
                  aria-current={selected.concept === c.id ? "page" : undefined}
                  className={cn(
                    "flex h-7 items-center gap-2 px-4 text-[13px] hover:bg-muted",
                    selected.concept === c.id && "bg-muted",
                  )}
                >
                  <span className="truncate">{c.label}</span>
                  <span className="ml-auto truncate font-mono text-[11px] text-faint">{c.key}</span>
                </Link>
              ))}
              {hits.length >= 8 && (
                <p className="px-4 pt-2 text-[11px] text-faint">Showing the best matches — type more to narrow.</p>
              )}
            </>
          )}
        </div>
