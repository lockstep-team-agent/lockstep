...
async function main(): Promise<void> {
  const ids = await withSystem(async (tx) => {
    ...
    const m = one(
      await tx.insert(members).values({ orgId: org.id, principalId: pr.id, githubUserId: 1, githubLogin: "naman" }).returning(),
    );
  ...
  const d1 = await proposeDecision(ids.orgId, { ...base, scopeKind: "shared", scopeRef: "POST /orders", ruleText: "orders need idempotencyKey", baseVersion: 0 });
  checks.push(["propose v1 → open", d1.version === 1 && d1.status === "open"]);

  let stale = false;
  try {
    await proposeDecision(ids.orgId, { ...base, scopeKind: "shared", scopeRef: "POST /orders", ruleText: "conflicting", baseVersion: 0 });
  } catch (e) {
    stale = statusCodeOf(e) === 409;
  }
  checks.push(["CAS rejects stale base_version (409)", stale]);

  const d2 = await proposeDecision(ids.orgId, { ...base, scopeKind: "shared", scopeRef: "POST /orders", ruleText: "orders need idempotencyKey v2", baseVersion: 1 });
  ...
  const owned = await proposeDecision(ids.orgId, { ...base, scopeKind: "repo", scopeRef: "order-service", ruleText: "use UTC", baseVersion: 0 });
  ...
  const dep = await registerDependency(ids.orgId, { ...base, consumerRepoId: ids.repoId, producedSurface: "POST /orders" });
  checks.push(["dependency edge registered", !!dep.edgeId]);

  const owedChange = await recordChange(ids.orgId, { ...base, repoId: ids.repoId, summary: "tweak", surface: "GET /orders", riskTier: "owned" });
  checks.push(["owned change → published", owedChange.publishState === "published"]);

  const sharedChange = await recordChange(ids.orgId, { ...base, repoId: ids.repoId, summary: "contract change", surface: "POST /orders", riskTier: "shared", contractDelta: { added: ["idempotencyKey"] }, verified: true, verifiedAgainst: "ts:orders.ts" });
